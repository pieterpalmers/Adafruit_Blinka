...
class Pin:
    """A basic Pin class for use with FT2232H."""
    ...
    def __init__(self, pin_id=None):
        ...
        if not Pin.ft2232h_gpio:
            ...
            i2c = I2cController()
            i2c.configure("ftdi://ftdi:ft2232h/1")
            Pin.ft2232h_gpio = i2c.get_gpio()
        # check if pin is valid
        if pin_id:
            if Pin.ft2232h_gpio.all_pins & 1 << pin_id == 0:
    ...
    def init(self, mode=IN, pull=None):
        ...
        # FT2232H does't have configurable internal pulls?
        if pull:
            raise ValueError("Internal pull up/down not currently supported.")
        pin_mask = Pin.ft2232h_gpio.pins | 1 << self.id
        current = Pin.ft2232h_gpio.direction
        if mode == self.OUT:
            current |= 1 << self.id
        else:
            current &= ~(1 << self.id)
        Pin.ft2232h_gpio.set_direction(pin_mask, current)

    def value(self, val=None):
        """Set or return the Pin Value"""
        if not self.id:
            raise RuntimeError("Can not access a None type pin.")
        current = Pin.ft2232h_gpio.read(with_output=True)
        # read
        if val is None:
        ...
        if val in (self.LOW, self.HIGH):
            ...
            Pin.ft2232h_gpio.write(current & Pin.ft2232h_gpio.direction)
            return None
        # release the kraken
